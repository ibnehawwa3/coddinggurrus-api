...
{
    public class ProblemContentRepository : BaseRepository, IProblemContentRepository
    {
        public ProblemContentRepository(IConfiguration config, IHttpContextAccessor httpContextAccessor) : base(config, httpContextAccessor)
        {
        }
        /// <summary>
        /// 
        ...
        public async Task<int> AddContent(Content content)
        {
            ...
            string sql = @"
            INSERT INTO dbo.ProblemContent (TopicId, Title,Text,IsActive)
        ...
        public async Task<IEnumerable<Content>> GetContents(ListingParameter listingParameter)
        {
            var countSql = @"SELECT COUNT(*) 
                     FROM dbo.ProblemContent a with (nolock) 
        ...
        public async Task<ContentViewModel> GetContentById(long id)
        {
            ...
            var sql = @"SELECT Id, Title, TopicId,Text FROM ProblemContent WHERE Id = @Id";

            var content = await connection.QuerySingleOrDefaultAsync<Content>(sql, new { Id = id });

            var sqlQuery = @"Select CourseId from ProblemTopic where Id= @Id";

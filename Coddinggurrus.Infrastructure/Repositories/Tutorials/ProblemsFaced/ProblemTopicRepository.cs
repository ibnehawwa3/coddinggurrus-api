...
namespace Coddinggurrus.Infrastructure.Repositories.Tutorials.ProblemsFaced
{
    public class ProblemTopicRepository : BaseRepository, IProblemTopicRepository
    {
        public ProblemTopicRepository(IConfiguration config, IHttpContextAccessor httpContextAccessor) : base(config, httpContextAccessor)
        {
        }
        /// <summary>
        /// 
        ...
        public async Task<int> AddTopic(Topic topic)
        {
            ...
            string sql = @"
            INSERT INTO dbo.ProblemTopic (Title, Description,CourseId,IsActive)
        ...
        {
            var sql = @"SELECT Id, Title, Description, CourseId FROM ProblemTopic WHERE Id = @Id";
        ...
        public async Task<IEnumerable<TopicCount>> GetTopics(ListingParameter listingParameter)
        {
            var countSql = @"SELECT COUNT(*) 
                     FROM dbo.ProblemTopic a with (nolock) 
                     WHERE (@TextToSearch IS NULL OR a.Title like '%' + @TextToSearch + '%')";
        ...
        FROM dbo.ProblemTopic with (nolock)
